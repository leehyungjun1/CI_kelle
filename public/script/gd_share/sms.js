        ...
        $form.on('click', selector, function () {
            ...
            if ($radio.val() === 'each') {
                ...
                window.addEventListener('message', function(event) {
                    ...
                    $(".js-receive-total").data('receive-total', receivedData.memberCount);
                    $(".js-receive-total").html(receivedData.memberCount);
                    
                    $(".js-reject-count").data('reject-count', receivedData.optOutMemberCount);
                    $(".js-reject-count").html(`(수신거부 대상자 ${receivedData.optOutMemberCount} 명 포함)`);

                    $("#member_groupLayer").addClass('active');
                    let compiledTemplate = _.template($('#templateCrmGroupItem').html());
                    $("#member_groupLayer").html(compiledTemplate(receivedData));
                });
            } else {
